...
cv_data_weekly %>% 
  filter(week_number >= 12 & sex == "all" & age == "all" &
           cause_of_death %in% c("covid-19-related", "all") & 
           location_of_death %nin% c("other-institution", "all")) %>%
  group_by(cause_of_death, location_of_death) %>%
  summarise(value = sum(value)) %>% 
  pivot_wider(names_from = "cause_of_death", values_from = "value") %>% 
  mutate(pct_covid = round(`covid-19-related`/sum(`covid-19-related`)* 100,1),
         pct_all = round(`covid-19-related`/all* 100, 1)) %>% 
  pivot_longer(c(pct_all, pct_covid), names_to = "measure", values_to = "value") %>%
  mutate(location_of_death = factor(location_of_death,
  ...
  ggplot(aes(location_of_death, value, fill = measure)) +
