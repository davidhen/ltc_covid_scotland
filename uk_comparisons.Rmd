...
```{r, warning=FALSE, message=FALSE}
#A gold star to anyone who can make the range updates for the 3 different Excel files for E&W, 
#Scotland & NI automatic.
...
data.sex.EW$age <- factor(data.sex.EW$age...1, levels=c("Under 1 year", "01-14", "15-44", "45-64", "65-74", "75-84", "85+", "Total"))
...
data.sex.EW.old <- data.sex.EW.old %>%
  group_by(weekno, sex...2) %>%
  summarise(max=max(deaths), min=min(deaths), mean=mean(deaths))

data.sex.EW.new <- merge(data.sex.EW.new, data.sex.EW.old, by=c("weekno", "sex...2"))
...
#Extract max/min values
#split off 2020 data
data.sex.age.EW.new <- subset(data.sex.EW, year==2020 & !age...1 %in% c("Total", "Under 1 year", "01-14", "15-44"))
data.sex.age.EW.old <- subset(data.sex.EW, year<2020 & !age...1 %in% c("Total", "Under 1 year", "01-14", "15-44"))

data.sex.age.EW.old <- data.sex.age.EW.old %>%
  group_by(weekno, sex...2, age) %>%
  summarise(max=max(deaths), min=min(deaths), mean=mean(deaths))

data.sex.age.EW.new <- merge(data.sex.age.EW.new, data.sex.age.EW.old, by=c("weekno", "sex...2", "age"))
...
#  geom_line(data=data.sex.age.EW.old, aes(x=weekno, y=mean), colour="Grey50", linetype=2)+      geom_line(data=data.sex.age.EW.new, aes(x=weekno, y=deaths), colour="Red")+
#  theme_classic()+
#  facet_grid(age~sex)+
#  scale_x_continuous(name="Week number", breaks=c(0,10,20,30,40,50))+
#  scale_y_continuous(name="Deaths registered")+
...
data %>% 
  ...
  mutate(diff = curr - hist,
         pct_change = round(diff / hist * 100, 1)) %>% 
  filter(location %in% c("Hospice", "Elsewhere", "Other communal establishment")) %>% 
  summarise(n = round(mean(diff, na.rm = TRUE),1)) %>% 
  pull(n) -> other_diff
...
data %>% 
  ...
  mutate(diff = curr - hist,
         pct_change = round(diff / hist * 100, 1)) %>% 
  filter(location %in% c("Hospital", "Care home", "Home")) %>% 
  ggplot(aes(week, pct_change, fill = location)) + 
  geom_col() +
  facet_wrap(~location) +
  ...
  labs(x = "Week number", y = "", subtitle =  "England and Wales") -> ew
  #labs(title = "All deaths in England & Wales by location",
  #     subtitle = "Difference from weekly average 2015-2019",
  #     x = "Week number",
  #     y = "",
  #     caption = glue::glue("Source: ONS\nExcluding other places of death where difference = +{other_diff} in weeks 11-23")) 
```

```{r}
main_tab <- feather::read_feather("derived_data/main_tab.feather")
```

```{r}
other_inst_diff <- 
  main_tab %>% 
  filter(location == "Other institution" & week_number >= 11) %>% 
  mutate(mean_deaths = round(mean_deaths, 0), 
         diff = deaths_all_2020 - mean_deaths,
         pct_change = round(diff / mean_deaths * 100, 1)) %>% 
  summarise(diff_tot = round(mean(pct_change, na.rm = TRUE),1)) %>% 
  pull(diff_tot)
...
main_tab %>% 
  ...
  theme(legend.position = "none",
        ...
        axis.title.x = element_text(size = 10)) +
  labs(subtitle = "Scotland", x= "Week number", y = "") -> sc
```
...
ew / sc +
  plot_annotation(
    title = "Percentage change in 2020 weekly all-cause deaths\nfrom previous 5 year average",
    caption = glue::glue("Source:ONS & NRS\nExcluding other places of death in E & W where average % difference = +{other_diff} in weeks 11-23\nExcluding other institutional place of death in Scotland where average % difference ={other_inst_diff} in weeks 11 to 24\n\U00A9 David AG Henderson")) +
  theme(plot.caption = element_text(size = 3)) -> fig_uk_excess
fig_uk_excess
```

```{r, echo=FALSE, eval=FALSE}
ggsave("plots/20200629_fig_uk_excess.png", fig_uk_excess, width = 12, height = 9)
```
